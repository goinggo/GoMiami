// This program demostrates how to manipulate slices and
...
func main() {
	...
	fmt.Printf("Display Original Slice\n")
	InspectSlice(orgSlice)

	fmt.Printf("From Org Slice: Slice 2 elements between indexes 2 and 3\n")
	slice2 := orgSlice[2:4]
	InspectSlice(slice2)

	fmt.Printf("From Slice 2: Slice index position 1 up to the capacity of slice2\n")
	slice3 := slice2[1:cap(slice2)]
	InspectSlice(slice3)

	fmt.Printf("From Slice 3: Change the value of index 0 in slice 3\n")
	slice3[0] = "CHANGED"
	InspectSlice(slice3)

	fmt.Printf("Display slice 2\n")
	InspectSlice(slice2)
}
